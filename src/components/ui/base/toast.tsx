...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-xl border p-4 pr-8 shadow-lg transition-all',
  {
    variants: {
      variant: {
        default: 'border-ink-200 bg-white text-ink-900 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-100',
        destructive: 'border-red-500/50 bg-red-500 text-white dark:border-red-900 dark:bg-red-900',
        success: 'border-success-500/50 bg-success-500 text-white dark:border-success-900 dark:bg-success-900',
        warning: 'border-warning-500/50 bg-warning-500 text-white dark:border-warning-900 dark:bg-warning-900',
      },
    },
    defaultVariants: {
      variant: 'default',
...
const ToastAction = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Action>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Action>>(
  ({ className, ...props }, ref) => (
    <ToastPrimitives.Action
      ref={ref}
      className={cn(
        'inline-flex h-8 shrink-0 items-center justify-center rounded-lg border border-transparent bg-ink-100 px-3 text-sm font-medium transition-colors hover:bg-ink-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 dark:bg-ink-800 dark:hover:bg-ink-700',
        className
      )}
      {...props}
    />
...
const ToastClose = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Close>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Close>>(
  ({ className, ...props }, ref) => (
    <ToastPrimitives.Close
      ref={ref}
      className={cn(
        'absolute right-2 top-2 rounded-lg p-1 text-ink-500 opacity-0 transition-opacity hover:text-ink-900 focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 dark:text-ink-400 dark:hover:text-ink-100',
        className
      )}
      toast-close=""
      {...props}
